...
class Akun extends CI_Controller {
	...
	function tambah() {
		...
		$this->form_validation->set_rules('kode_akun', 'Kode Akun', 'required|min_length[4]|max_length[4]');
		...
		if($btnSimpan=="simpan") {
			if ($this->form_validation->run() == FALSE) {
				...
				if($this->model_akun->check_kode_akun_insert($kode_check)>0) {
					...
					$data_akun = array("id_tipe_akun" => $data["id_tipe_akun"], "kode_akun" => $data["kode_akun_depan"].$data["kode_akun"], "nama_akun" => $data["nama_akun"], "saldo_normal" => $data["saldo_normal"], "lokasi" => $data["lokasi"], "posisi" => $data["posisi"], "aktif" => "A", "company_id" => $this->session->userdata($this->config->item('ses_company_id')) );
					$data['id_tipe_akun'] 		= "";
	...
	function ubah() {
		...
		$data['akun'] 		= $this->model_akun->get_akun_by_id($id_akun)->row_array();
		$data['tipeakun'] 	= $this->model_tipeakun->list_tipeakun()->result();
		$data['errors'] 	= '';
		$data['id_tipe_akun'] 		= $this->input->post('tipe_akun');
		$data['kode_akun_depan'] 	= $this->input->post('kode_akun_depan');
