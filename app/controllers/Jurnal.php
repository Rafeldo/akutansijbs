...
class Jurnal extends CI_Controller {
	...
	function index() {
		...
		$data['jurnal'] 		= $this->model_jurnal->list_jurnal($data['tanggal_awal'], $data['tanggal_akhir'], $data['no_jurnal'], $data['dikonfirmasi'])->result();
	...
	function tambah() {
		...
		$tmp_no_jurnal 		= $this->input->post('no_jurnal');
		$data['total'] 		= $this->input->post('total');
		$data['total'] 		= str_replace(".", "", $data['total']);
		$data['total'] 		= str_replace(",", ".", $data['total']);
		$data['errors'] 	= '';
		if($data['no_jurnal']=="" || $data["tgl_jurnal"]=="") {
			...
			if($cekTutupBuku>0) {
				...
				$data_jurnal = array("no_jurnal" 	=> $data['no_jurnal'],
											...
											//"memo" 			=> $data['memo'],
											"tgl_jurnal" 	=> date_format(date_create($data['tgl_jurnal']), "Y-m-d H:i:s"),
											"total" 		=> $data['total'],
											"dikonfirmasi" 	=> "Yes",
											"yang_buat" 	=> "0",
											"jenis_jurnal" 	=> "JU",
											"kode_unik" 	=> $kode_unik, "company_id" 	=> $this->session->userdata($this->config->item('ses_company_id')) );
				...
				for ($i=0; $i<$baris; $i++) {
					...
					$kredit 	= str_replace(",", ".", $kredit);
					$amount = $debet - $kredit;
					$data_detail = array("id_jurnal" => $id_jurnal, "id_akun" 	=> $this->input->post('akun')[$i], "amount" 	=> $amount, "uraian" 	=> $uraian, "debet" 	=> $debet, "kredit" 	=> $kredit, );
	...
	public function ubah() {
		...
		if($no_jurnal=="" || $tgl_jurnal=="") {
			...
			if($cekTutupBuku>0) {
				...
				$data_jurnal = array("no_jurnal" 	=> $no_jurnal,
											//"no_bukti" 		=> $no_bukti,
											//"memo" 			=> $memo,
											"tgl_jurnal" 	=> date_format(date_create($tgl_jurnal), "Y-m-d H:i:s"),
											"total" 		=> $total,
				...
				$this->session->set_userdata($this->config->item('ses_message'), $message);
				redirect('jurnal/detail/'.$kode_unik);
			}
		}
	...
	public function confirm() {
		...
		$sql 	= "update trs_jurnal set ";
		$sql 	.= " dikonfirmasi = 'Yes' ";
	...
	public function delete($id) {
		 $row = $this->model_jurnal->get_data_detail($id);

		 if ($row) {
			  $this->model_jurnal->delete($id);
