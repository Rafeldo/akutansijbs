						...
						<div class="form-group">
							...
							<div class="col-lg-4">
								<select class="form-control selectnot" data-validetta="required" name="tipe_akun" data-placeholder="pilih tipe akun" onchange="js_get_kode_tipe_akun(this.value)">
									<option value=""></option>
									<?php foreach ($tipeakun as $tp) {$sel = ""; $ket = "(". $tp->kode_tipe_akun .") - " . $tp->nama_tipe_akun; if($akun["id_tipe_akun"] == $tp->id_tipe_akun) $sel = 'selected=""'; echo '<option value="'.$tp->id_tipe_akun.'" '.$sel.'>'.$ket.'<option>'; } ?>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-lg-2"> <div class="input-group">
								<span class="input-group-addon" id="kode_akun_depan"><?=$kode_akun_depan?></span>
								<input type="hidden" name="kode_akun_depan" id="value_kode_depan" value="<?=$kode_akun_depan?>">
								<input type="text" class="form-control" data-validetta="required" maxlength="4" name="kode_akun" id="kode_akun" value="<?=$kode_akun?>" />
							</div>
						</div>
					</div>
					<div class="form-group">
