...
<section class="content">
	<div class="row">
		<div class="col-lg-12">
			<?php if($errors!="") echo $errors; if($this->session->userdata($this->config->item('ses_message'))) {echo $this->session->userdata($this->config->item('ses_message')); $this->session->unset_userdata($this->config->item('ses_message')); } $display = ""; if($mobile==1) {$display = "hidden"; echo alert_php2('Peringatan.', 'danger', '<br/>Sangat disarankan menggunakan laptop atau pc untuk menginput jurnal'); } ?>
						...
						<div class="row">
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<input type="text" class="form-control" data-validetta="required" maxlength="40" name="no_jurnal" id="no_jurnal" value="<?=$no_jurnal?>" onchange="form_input(true)" />
								</div>
							</div>
...
function tambah_baris_baru()
{
	...
	else
	{
		...
		var html = '<tr>';
		html += '<td width="10%"><span class="no-bukti">'+ no_jurnal +'</span></td>';
