...
class Model_jurnal extends CI_Model {
	...
	function get_data_detail($id) {
		$this->db->select('*');
		$this->db->where('trs_jurnal_detail.id_jurnal', $id);
	...
	function get_no_jurnal() {
		...
		$this->db->where('MONTH(trs_jurnal.tgl_jurnal)', date('m'));
		$this->db->where('YEAR(trs_jurnal.tgl_jurnal)', date('Y'));
		$this->db->where('company_id', $this->session->userdata($this->config->item('ses_company_id')));
		$query 	= $this->db->get('trs_jurnal');
		$data 	= $query->row_array();
		$no 	= "JL-" . date('Ym-') . substr("00000".$data['noj'], -5);
		return $no;
	}
